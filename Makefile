...
build-docker-image:	# Build the docker image
	docker build -f docker/csl/Dockerfile -t $(IMAGE_NAME)-api:$${IMAGE_TAG:-latest} --platform linux/amd64 .
	docker build -f docker/csl-python/Dockerfile -t $(IMAGE_NAME)-data-import:$${IMAGE_TAG:-latest} --platform linux/amd64 ./docker/csl-python

push-docker-image:	# Push the docker image to ECR
	docker tag "$(IMAGE_NAME)-api:$${IMAGE_TAG}" "$(ECR_WEST_DOMAIN)/$(IMAGE_NAME)-api:$${IMAGE_TAG}"
	docker push "$(ECR_WEST_DOMAIN)/$(IMAGE_NAME)-api:$${IMAGE_TAG}"
	docker tag "$(IMAGE_NAME)-data-import:$${IMAGE_TAG}" "$(ECR_WEST_DOMAIN)/$(IMAGE_NAME)-data-import:$${IMAGE_TAG}"
